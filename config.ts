...
import { validateServerPolicy, type ToolPolicy } from "./policy.ts";
...
export function getMcpDiscoverySummary(overridePaths?: string | string[], cwd = process.cwd()): McpDiscoverySummary {
  const sources = getConfigSourcesForPaths(normalizeOverridePaths(overridePaths), cwd).map((source) => {
...
export function loadMcpConfig(overridePaths?: string | string[], cwd = process.cwd()): McpConfig {
  let config: McpConfig = { mcpServers: {} };

  for (const source of getConfigSourcesForPaths(normalizeOverridePaths(overridePaths), cwd)) {
    const loaded = readValidatedConfig(source.readPath, `MCP config from ${source.readPath}`);
    if (!loaded) continue;
...
export function getServerProvenance(overridePaths?: string | string[], cwd = process.cwd()): Map<string, ServerProvenance> {
  ...
  for (const source of getConfigSourcesForPaths(normalized, cwd)) {
    const loaded = readValidatedConfig(source.readPath, `MCP config from ${source.readPath}`);
    if (!loaded) continue;
